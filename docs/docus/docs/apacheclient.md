...
However, you may want to switch to `ApacheHttp5Sender`, backed by `org.apache.hc.client5.http.classic.HttpClient`.
...
Then you can explicitly specify `ApacheHttp5Sender` by setting `class` attribute for `http.sender` section:
...
There are some specific settings available only for `ApacheHttp5Sender` that you can specify in `sender` section as well:
